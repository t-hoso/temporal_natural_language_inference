...
sys.path.append(".")
from .load_data import load_fold_sentences, load_explain_split_data, train_test_fold_split
from .mode import Mode

class ExplainKnowledgeDataset(torch.utils.data.Dataset):
    def __init__(self, mode, fold, val=True):
        super().__init__()
    ...
    def __getitem__(self, item):
        return torch.LongTensor(self.result[0][item]), torch.LongTensor([self.result[2][item]]),\
               torch.LongTensor([self.result[1][item]]), self.result[3][item], self.result[4][item]
